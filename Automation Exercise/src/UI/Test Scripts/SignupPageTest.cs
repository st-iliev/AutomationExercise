...
namespace Automation_Exercise.Test_Scripts
{
    ...
    public class SignupPageTest : BaseTest
    {
        ...
        [Test, Order(4)]
        [TestCaseSource(typeof(SignupTestCases), nameof(SignupTestCases.ErrorMessagesOnSignup))]
        public void VerifyErrorMessageDisplayedOnRequiredFields(string name, string password, string firsName, string lastName, string address1, string country, string state, string city, string zipeCode, string mobileNumber)
        {
            test = suiteTest.CreateNode("Test Error Message Displayed On Required Fields");
            accountInfo = new AccountInfo()
            {
                Name = name,
                Password = password,
            ...
            if (accountInfo.Name != "")
            {
                signupPage.AssertValidationMessageIsDisplayed(signupPage.nameField);
            }
